...
class AudioCNN(nn.Module):
    def __init__(self,num_classes=50) -> None:
        ...
        self.layer1 = nn.ModuleList(
            [ResidualBlock(in_channels=64,out_channels=64) for i in range(3)])
    ...
    def forward(self,x,return_feature_maps=False):
        if not return_feature_maps:
            ...
            for i,block in enumerate(self.layer1):
                x= block(x,fmap_dict=feature_maps,prefix=f"layer1.block{i}")
            feature_maps['layer1'] = x

            for i,block in enumerate(self.layer2):
                x = block(x,fmap_dict=feature_maps,prefix=f"layer2.block{i}")
            feature_maps['layer2'] = x

            for i,block in enumerate(self.layer3):
                x = block(x,fmap_dict=feature_maps,prefix=f"layer3.block{i}")
            feature_maps['layer3'] = x

            for i,block in enumerate(self.layer4):
                x = block(x,fmap_dict=feature_maps,prefix=f"layer4.block{i}")
